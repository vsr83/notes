...
Especially section \ref{sec:spectrum_and_radiation} is very likely to contain 
errors. Also, I haven't compared the 3d visualization results to anything in
...
it can be only detected using Very Long Baseline Interferometry (VLBI).
...
\begin {figure}[h]
  \begin {center}
     \includegraphics [angle=0, width=1.0\columnwidth] {fig/doppler_limits.eps}
...
\subsection{Black-Body Radiance and Stars}
...
spectrum, the computation of the Doppler effect is no longer trivial. In a 
continuous spectrum, we do not expect a radiance that transforms proportionally 
to fourth power of the frequency: If the Doppler effect indicates a ratio 
$\omega/\omega'$ ratio of frequencies between  $S$ and $S'$ spectrum in the range 
$[\omega', \omega' + \Delta']$ in $S'$ will correspond to spectrum in the range 
$[\omega, \omega + (\omega/\omega')\Delta']$ in $S$. This suggests that radiance 
transforms instead to the third power.
