...
\title{Optical Effects from Observer Motion}
\author{Ville R{\"a}is{\"a}nen\thanks{\url{https://www.github.com/vsr83}}}
%\subtitle{Notes}
...
and implementation of aberration correction algorithms in software.
Significant emphasis is placed on how results are derived and on visualizations 
that attempt help development of intuition. Since the document was mostly produced 
by a personal attempt to (re)discover the relevant results, there will be very 
few references to the existing literature on the topic. Experimental evidence 
is not discussed at all. 

This document is divided into three chapters:
\begin {itemize}
\item Chapter 1 discusses aberration, derivation of different useful formulas 
and application of perspective projection to the visualization of distortion 
resulting from relativistic aberration.
\item Chapter 2 discusses the perception of color, the Doppler Effect and 
basics of ray-tracing in the context of special relativity.
\item Chapter 3 uses basic concepts of radiometry and Lorentz transformations of 
electromagnetic fields to discuss brightness and transformation of 
continuous spectra. 
\end {itemize}
A word of caution: Since I'm not an expert on any of the topics discussed in 
this document, it is very likely that there are serious technical errors. 
Also, I haven't compared the 3d visualization results to anything in
the literature or online video platforms.
...
\begin {figure}[t]
  ...
  \end {center}
\end {figure}
A powerful introductory insight into many theories of physics can be gained from simple 
thought experiments involving everyday things and phenomena often in extreme circumstances:
For example, standard introduction to the theory of special relativity often is 
presented using thought experiments involving trains moving at relativistic velocities 
and flashes of lightning. In a similar vein, we introduce the phenomenon of aberration 
with a thought experiment, where the effect of finite speed is very apparent and 
intuitive.
...
expressed 
\begin {eqnarray}
    \label{eq:classical_aberration1}
    \boxed{
        \quad
        \tan\theta' 
        = 
...
\end {eqnarray}
where we have defined the \textbf{beta factor} $\beta: = v/u$ and 
$\sec\theta = 1/\cos\theta$. This classical formula is limited angles in $+x$ 
direction. More generally, for an arbitrary direction
\begin {eqnarray}
    \label{eq:classical_aberration2}
    \boxed{
        \quad
        \theta' 
        =
...
\end {eqnarray}
The equations $(\ref{eq:classical_aberration1})$ and $(\ref{eq:classical_aberration2})$ 
perfectly describe classical aberration and are accurate enough for most practical 
cases. 
\begin {figure}
  \begin {center}
  ...
  \end {center}
\end {figure} 
Now consider the special case of light with $u=c$ in the context of Special Relativity. 
Then, a simple application of Lorentz transformation yields 
\begin {eqnarray}
    \begin {bmatrix}
        t' \\ x' \\ y' \\ z'
    \end {bmatrix}
...
\end {eqnarray}
After substitution of trajectory 
$(x(t), y(t), z(t)) = (-ct\cos\theta, -ct\sin\theta, 0)$, we get
\begin {eqnarray}
    x' &=& \gamma(x - vt) 
...
\begin {eqnarray}
    \label{eq:relativistic_aberration2}
    \theta' 
    =
...
(see Figure \ref{fig:classicrel}). This is called the \textbf{headlight effect} or 
\textbf{relativistic beaming}.

The aberration corrections are always rotationally symmetric w.r.t. axis 
collinear to the direction of motion of the observer. That is, if we choose 
a spherical coordinate system with a pole in the direction of motion, the 
correction will be independent of longitude (see Figure \ref{fig:3d}). That is, 
the correction will always map all directions on a line of latitude to 
another line of latitude without changing longitude.
...
\subsection {Aberration Formulas}
Even though $(\ref{eq:relativistic_aberration2})$ describes the relativistic 
aberration in all cases, for practical purposes it is often more convenient to 
use vectors. Also for most real-world applications, $\beta$ is small enough so 
that linear or quadratic approximations can be applied with sufficient accuracy. 
In astronomy, expressions for aberration also in terms of spherical coordinates are 
often used.

\subsubsection{General Vector Expression for Aberration}
Any vector expression for aberration will involve addition of velocity vectors,
...
called the \textbf{proper direction}. In special relativity, the speed of light 
is equal to $c$ in both frames. However, classical expression for the proper 
direction needs to take into account that classical addition of velocities can 
lead to velocity of light greater than $c$. 
The \textbf{uncorrected direction} in both cases can be written $\vu p = -\vu u/c$. 
...
In order to derive a vector formula for relativistic aberration, we need an expression 
for general Lorentz transformation of velocities. We start by considering an inertial 
frame $S'$ moving w.r.t. $S$ with velocity $\vc v$ and denote 
\begin {eqnarray}
    \vc u := \diff{\vc r}{t}, 
    \quad
    \vc u' := \diff{\vc r'}{t'},.
...
\begin {eqnarray}
    ...
    \right\}.
\end {eqnarray}
After division by $-c$, we get the expression
\begin {eqnarray}
    \vu s 
    &=&
    \frac{1}{\gamma(1 - \vu n\cdot\vc v/c)}
...
\end {eqnarray}
This is the most general relativistic aberration formula equivalent to 
$(\ref{eq:relativistic_aberration2})$. Application to any light source requires simply 
the substitution of the direction $\vu p$ to target and vector beta factor $\vc\beta$.
The result then points to observed direction to target. In the limit $\beta\to 0$, we get 
$\vu s\to\vu p$ and in the limit $\beta\to 1$, $\vu s\to\vu\beta$. The same vector 
formula (with different notation) can be found from \cite{almanac}.

\subsubsection{Series Expansion of Aberration Correction}
%\subsubsection{Classical Aberration}
In most practical applications, $\beta$ is small enough so we can approximate even the 
classical correction with simpler formulas. 
...
Let's now consider the relativistic case.
It actually turns out that even the quadratic approximations are incompatible for 
classical and relativistic expansions. Consequently, one should only apply the linear 
part of $(\ref{eq:classical_series})$.
%\subsubsection{Relativistic Aberration}
...
around $2$ mas and $0.2$ $\mu$as, respectively. This error is so small, that 
it could be only detected using Very Long Baseline Interferometry (VLBI).
...
In many software implementations and textbooks, approximate aberration corrections 
are applied in terms of spherical coordinates. We expand uncorrected direction 
\begin {eqnarray}
    \vu p
    = 
    \begin {bmatrix}
...
\end {eqnarray}
This is equivalent to the equation $(23.4)$ in \cite{meeus}.
%\newpage
...
\begin {itemize}
    ...
    due to 225-million year orbit, the effect is included in the catalogued 
    positions and is not corrected.
...
on Earth, the maximum aberration is around $5.5$ arcseconds. Not correcting 
for this together with annual aberration can deteriorate the accuracy of 
star trackers responsible for providing inputs to satellite attitude control.

The expressions developed in the previous section apply to all types of 
aberration. 

\subsubsection{Stellar and Diurnal Aberration in Spherical Coordinates}
%\subsubsection{Annual Aberration}
For elliptic orbits, the velocity and distance from the focus can be 
expressed 
\begin {eqnarray}
    v 
    =
...
\end {eqnarray}
Note that $\delta$ and $\alpha$ are now spherical coordinates w.r.t. the ecliptic
instead of the equator. Suppose $\alpha=0$. Then, the above simplifies to 
\begin {eqnarray}
    \Delta\delta &=& 
...
with a period of one orbit (see Figure \ref{fig:aberration_ellipse}). For a star 
is on ecliptic plane, the ellipse is degenerate. At zenith w.r.t. ecliptic plane, 
...
Let's now consider diurnal aberration.
...
\subsubsection{Planetary Aberration and Light-Time}
...
are discarded in a process called \textbf{culling}. 
...
Perspective projection is used to map points in the three-dimensional scene via
normalized device coordinates to pixel coordinates on a screen or some other
image system. All points from the same direction in the frustrum will be always 
mapped to location on the screen. Aberration correction (e.g. $(\ref{eq:properdir2})$) 
for the direction from the camera to the point on the scene is applied before 
...
$(\alpha x_c, \alpha y_c, \alpha z_c, \alpha)$ are considered equivalent for all non-zero 
...
\lstset{numbers=left, basicstyle=\footnotesize,breaklines=true}
In the visualizations shown in this section, we use an implementation that 
combines the symmetric infinite special case $(\ref{eq:syminfinite_proj})$
of a projection matrix with a mapping to normalized device coordinates via 
division by $w_c = -z_e$. We also reorient axes so that the camera points 
to +z direction and positive $x_e, y_e$ will map to positive $x_n, y_n$.

The second listing implements the vector aberration corrections 
$(\ref{eq:properdir2})$ and $(\ref{eq:classical_properdir})$.

\begin{lstlisting}[language=python, caption={Projection on world coordinates into screen or image coordinates.}]
import numpy as np

def project_ndc(p, n, w, h):
    """Project point from the frustrum starting from [-w, w] x [-h, h] x {n} to [-1, 1] x [-1, 1]"""
    x_n = (n * p[0]) / (w * p[2])
    y_n = (n * p[1]) / (h * p[2])

    return x_n, y_n

def ndc_to_screen(x_n, y_n, x_min, x_max, y_min, y_max):
    """Convert position from NDC coordinates to screen coordinates"""
    x_s = 0.5 * ((x_max + x_min) + (x_max - x_min) * x_n)
    y_s = 0.5 * ((y_max + y_min) + (y_max - y_min) * y_n)

    return x_s, y_s
\end{lstlisting}

\begin{lstlisting}[language=python, caption={Relativistic and classical aberration corrections.}]
import numpy as np

def aberrate_direction_rel(p, beta_vec):
    """Apply relativistic aberration to the direction of a position vector."""

    beta = np.linalg.norm(beta_vec)
    gamma = 1 / np.sqrt(1 - beta * beta)
    pu = p / np.linalg.norm(p)
    
    su = 1 / (gamma * (1 + np.dot(pu, beta_vec))) * (pu + gamma * beta_vec * 
        (1 + np.dot(pu, beta_vec) * gamma / (gamma + 1)))
    
    s = su * np.linalg.norm(p)
    return s

def aberrate_direction_cla(p, beta_vec):
    """Apply classical aberration to the direction of a position vector."""

    beta = np.linalg.norm(beta_vec)
    gamma = 1 / np.sqrt(1 - beta * beta)
    pu = p / np.linalg.norm(p)
    
    su = (pu + beta_vec) / np.linalg.norm(pu + beta_vec)
    
    s = su * np.linalg.norm(p)
    return s
\end{lstlisting}
\subsection{Deformation at Relativistic Speeds}
In this section, the techniques discussed in the previous section are applied to the 
visualization of deformation in the visual field resulting from relativistic 
aberration. We will focus on the deformation of spherical and rectangular grids with 
increasing $\beta$. The visualizations do not involve the perception of color or 
brightness, which will be discussed in the next chapter.
...
\begin {figure}
  \begin {center}
     \includegraphics [angle=0, width=0.9\columnwidth, clip=true, viewport=70 160 1100 1600] {fig/aberration_cubes.eps}
     \caption{\label{fig:aberration_cubes}Deformation of a set of cubes.}
  \end {center}
\end {figure} 

\newpage
\restoregeometry
\section{Spectrum, Color and the Doppler Effect}
The \textbf{Doppler effect} refers to how movement of an observer w.r.t. a light 
source influences the observed spectral content of the light. A human observer 
will typically perceive \textbf{Doppler shift} as a change in color and/or 
change in brightness. The source above or below human visual range can also become 
visible or a source inside human visual range can disappear from sight. We will
start this chapter by a discussion of human perception of color to obtain a clear 
understanding how perceived color relates to the spectral content of light and 
digital images. Next, the classical and relativistic Doppler effects are discussed 
in detail. Finally, we will discuss the basics of ray-tracing and apply very basic 
ray-tracer to the visualization of the interplay of relativistic aberration and 
the Doppler effect.
This chapter generally ignores how the interplay of aberration and Doppler 
effect influence brightness and focuses only on monochromatic sources of light when 
discussing the Doppler effect.

\subsection{Human Perception of Color}
The human visual system is rather limited in the perception of color: Our color 
vision cannot usually untangle the continuous spectrum of light responsible for 
the perception of specific color into its constituent wavelengths. Rather, our 
color vision is based on visual signals from three different types of cone cells 
on the retina each with their associated wavelength-dependent sensitivities. Thus, 
instead of reproducing entire continuous spectra, digital images, computer 
displays and printed images only need to reproduce the same perception of color. 
This can be achieved, for instance, with combinations of Red, Green and Blue (RGB).
This section will discuss the relationship between spectral 
content of light, human perception of color and SRGB values used in digital images.

\subsubsection{Human Vision and LMS Color Space}
Human vision is limited to the wavelength range between 380 and 700 nm and is 
\textbf{trichromatic}. That is, the perception of color is based on three classes 
of cone cells on the retina. Each type of cone cell has different \textbf{spectral 
sensitivity}. That is, they are sensitive to different ranges of wavelengths. We 
can refer to the three classes according to their peak wavelength as long (L), 
medium (M) and short (S) with peaks at 560, 530 and 420 nm, respectively.
...
called \textbf{metamers}. Human vision cannot differentiate between the color of 
metamers.
...
Note that different wavelengths (e.g. in the range $[650, 700]$ nm) can be perceived 
as the same color but with different brightness.
\begin {figure}
  \begin {center}
...
\subsubsection{Implementation}
\begin{lstlisting}[language=python, caption={Implementation of the integration of XYZ tristimulus 
    values and conversion to SRGB.}]
import numpy as np

# CIE tristimulus values from http://cvrl.ioo.ucl.ac.uk/cie.htm
# The Colour & Vision Research laboratory, Institute of Ophthalmology, University College London
CIE_data = np.array([
[360,0.000129900000,0.000003917000,0.000606100000],
...
[830,0.000001251141,0.000000451810,0.000000000000]]);

CIE_wavel = CIE_data[:, 0]
CIE_X = CIE_data[:, 1]
CIE_Y = CIE_data[:, 2]
CIE_Z = CIE_data[:, 3]

def cie_xyz_integrate(wavelength, power):
    """Integrate CIE tristimulus values.

    @param: wavelength Wavelength array (nm) corresponding to the power array values.
    @param: power      Spectral power distribution (W/m^2/nm) 
    @return: X, Y, Z   The CIE tristimulus values.
    """
    # Interpolate values on the CIE wavelength grid.
    power_interp = np.interp(CIE_wavel, wavelength, power)
    
    # Step size in CIE data equals to 1 nm.
    step_size = 1

    X = np.trapz(CIE_X * power_interp, CIE_wavel, step_size)
    Y = np.trapz(CIE_Y * power_interp, CIE_wavel, step_size)
    Z = np.trapz(CIE_Z * power_interp, CIE_wavel, step_size)

    return X, Y, Z

def cie_xyz_delta(wavelength):
    """Integrate CIE tristimulus values with dirac delta.

    @param: wavelength Wavelength for the dirac delta (nm).
    @return: X, Y, Z   The CIE tristimulus values.
    """

    X = np.interp(wavelength, CIE_wavel, CIE_X)
    Y = np.interp(wavelength, CIE_wavel, CIE_Y)
    Z = np.interp(wavelength, CIE_wavel, CIE_Z)

    return X, Y, Z


def srgbl_srgb(srgbl_value):
    """Convert SRGB-linear value to SRGB value
    
    @param srgbl_value: The SRGB linear value
    @return: The SRGB value in the range [0, 1] with gamma correction and clipping.
    """

    # Apply clipping.
    srgbl_value = max(0, srgbl_value)
    srgbl_value = min(1, srgbl_value)

    # Apply gamma or scaling.
    if srgbl_value < 0.0031308:
        value_out = 12.92 * srgbl_value 
    else: 
        value_out = 1.055 * (srgbl_value ** (1.0/2.4)) - 0.055
    return value_out


def cie_xyz_sgrb(X, Y, Z):
    """Convert CIE tristimulus values to SRGB values
    
    @param: X The CIE tristimulus value X
    @param: Y The CIE tristimulus value Y
    @param: Z The CIE tristimulus value Z
    @return R, G, B: The SRGB values.
    """

    srgbl_r =  3.2406255 * X - 1.5372080 * Y - 0.4986286 * Z
    srgbl_g = -0.9689307 * X + 1.8757561 * Y + 0.0415175 * Z
    srgbl_b =  0.0557101 * X - 0.2040211 * Y + 1.0569959 * Z

    srgb_r = srgbl_srgb(srgbl_r)
    srgb_g = srgbl_srgb(srgbl_g)
    srgb_b = srgbl_srgb(srgbl_b)
...
\subsubsection{Classical and Relativistic Doppler Effect}
\begin {figure}[t]
  \begin {center}
    \begin{tikzpicture}
...
\subsubsection{Longitudal and Transverse Motion}
...
a blueshift. In principle, a planet orbiting a star on a pecfectly circular 
orbit will experience Doppler blueshift of the starlight and a imaging 
satellite orbiting a planet will experience (Doppler) redshift (see 
Figure \ref{fig:doppler_transverse}). In practice, planetary orbits are 
typically elliptic enough so that the radial component of velocity 
will introduce a larger redshift.
...
\subsubsection{The Redshift-Blueshift Boundary and Limits of Vision}
...
or equivalently 
\begin {eqnarray}
    \gamma(1 - \beta\cos\theta_{limit}) = 1
    &\Leftrightarrow&
    \cos\theta_{limit} = \frac{1}{\beta}\left(1 - \frac{1}{\gamma}\right).
\end {eqnarray}
...
dot when $\beta\to 1$. The limit angle is also the only angle $\theta$ from 
which observer can observe the rest frame spectrum.  
\begin {figure}[h]
  \begin {center}
...
An additional consequence of the dependence of the Doppler shift on the angle 
$\theta$ is that a given wavelength $\lambda'$ in the source frame $S'$ is 
shifted into visible wavelengths $\lambda$
in $S$ for a limited range of angles $\theta$. Rearranging 
$(\ref{eq:doppler_relativistic})$, yields 
\begin {eqnarray}
    \cos\theta
    =
    \frac{1}{\beta}\left(1 - \frac{\omega'}{\gamma\omega}\right)
    =
    \frac{1}{\beta}\left(1 - \frac{\lambda}{\gamma\lambda'}\right).
\end {eqnarray}
Thus, for a source wavelength $\lambda'$, an observer capable of perceiving 
the wavelengths $\lambda\in[\lambda_{min}, \lambda_{max}]$ cannot observe
$\lambda$ shifted from $\lambda'$ at angles $\theta$ outside the range 
$[\theta_{min}, \theta_{max}]$, where 
\begin {eqnarray}
    \label{eq:visible_dopler_min}
    \theta_{min} &=& \arccos\left[
        \frac{1}{\beta}\left(1 - \frac{\lambda_{min}}{\gamma\lambda'}\right)
    \right],
    \\
    \label{eq:visible_dopler_max}
    \theta_{max} &=& \arccos\left[
        \frac{1}{\beta}\left(1 - \frac{\lambda_{max}}{\gamma\lambda'}\right)
    \right].
\end {eqnarray}
If we want to determine at what range of angles visible wavelengths in $S'$ 
are visible in $S$, we obtain with simple substitutions $\lambda'=\lambda_{max}$
and $\lambda'=\lambda_{min}$
\begin {eqnarray}
    \label{eq:visible_dopler_min2}
    \theta_{min} &=& \arccos\left[
        \frac{1}{\beta}\left(1 - \frac{\lambda_{min}}{\gamma\lambda_{max}}\right)
    \right],
    \\
    \label{eq:visible_dopler_max2}
    \theta_{max} &=& \arccos\left[
        \frac{1}{\beta}\left(1 - \frac{\lambda_{max}}{\gamma\lambda_{min}}\right)
    \right].
\end {eqnarray}
Figure \ref{fig:doppler_visibility} depicts angle ranges into which visible wavelengths 
are shifted into as visible light. An observer accelerating towards $\beta=1$ will
observe longer wavelengths $\lambda'$ (e.g. red) in $S'$ being visible longer at 
angles $\theta<\theta_{limit}$ as they require more Doppler shift to be blueshifted 
to $\lambda<\lambda_{min}$. Similarly shorter wavelengths $\lambda'$ (e.g. blue) 
will be visible longer at angles $\theta>\theta_{limit}$ since they require more 
Doppler shift to be redshifted to outside $\lambda>\lambda_{max}$.

Wavelengths from outside the visual range in $S'$ can be also shifted to wavelengths 
in the visual range in $S$ as shown in Figure \ref{fig:doppler_visibility2} for 
$200$ nm ultraviolet radiation and $900$ nm infrared radiation. As the velocity 
of the observer converges towards the speed of light, increasingly shorter/longer 
wavelengths will be redshifted/blueshifted into the visible range in $S$.
Around $\beta=0.999999$, even the cosmic microwave background will be blueshifted 
to the visible range \cite{blackbody_doppler2}.
\begin {figure}[h]
  \begin {center}
     \includegraphics [angle=0, width=1.0\columnwidth, viewport=45 30 592 580, clip=true] {fig/doppler_visibility.eps}
     \caption{\label{fig:doppler_visibility}Angles $\theta$ in $S$ into which the wavelengths 
     $650$ nm, $525$ nm and $475$ nm from $S'$ are shifted into as visible light. The 
     black curves depict the range of visible angles for all visible wavelengths in $S'$. 
     The dotted black curve depicts the boundary between redshift and blueshift. }
  \end {center}
\end {figure} 
\FloatBarrier
\begin {figure}[h]
  \begin {center}
     \includegraphics [angle=0, width=1.0\columnwidth, viewport=40 10 592 300, clip=true] {fig/doppler_visibility2.eps}
     \caption{\label{fig:doppler_visibility2}Angles $\theta$ in $S$ into which the wavelengths 
     $200$ nm and $900$ nm from $S'$ are shifted into as visible light. The 
     black curves depict the range of visible angles for all visible wavelengths in $S'$. 
     The dotted black curve depicts the boundary between redshift and blueshift. }
  \end {center}
\end {figure} 
...
\subsubsection{Viewport and Rays}
In order to determine an SRGB value for each pixel in an image, we start 
ray tracing by mapping each pixel to an aberrated direction vector. The 
aberrated vector is then transformed to a non-aberrated vector using inverse 
aberration formula. That is, instead of using the combination of perspective 
projection and aberration correction, in ray tracing we apply inverse steps 
in inverse order.

Consider the geometry of the viewport shown in Figure \ref{fig:viewport}.
Let $\vc E$ be the camera location, $\vu t$ the target direction, 
$\vc C = \vc E + d\:\vu t$ the center of the viewport and $\theta$ the 
...
\begin {figure}[t]
  \begin {center}
    \begin{tikzpicture}
        ...
        \draw[dashed] (0, 0) -- (4, 0);
        \draw[thick, ->] (4, 0) -- (7, 0);
        \draw[thick] (0,0) -- (4 - 1.0, -1.0);
        \draw[thick] (0,0) -- (3, 0.6);
        \draw[dashed] (3,0.6) -- (5, 1.0);

        \draw (5.6, 2.25) node{$\vc P_{11}$};
        \draw (5.6, -0.25) node{$\vc P_{1m}$};
        \draw (2.35, -0.4) node{$\vc P_{k1}$};
        \draw (2.35, -2.4) node{$\vc P_{km}$};
        \draw (-0.5, 0) node{$\vc E$};
        \draw (0, 0) node{$\bullet$};

        \foreach \dy in {-1,...,1} {
            \foreach \dx in {-2,...,2} {
                \draw[thick] (4.0 - 0.25 + 0.5 * \dx, -0.75 + 0.5 * \dx + \dy) -- (4.0 - 0.25 + 0.5 * \dx, 0.25+ 0.5 * \dx + \dy);
                \draw[thick] (4.0 + 0.25 + 0.5 * \dx, -0.25 + 0.5 * \dx + \dy) -- (4.0 + 0.25 + 0.5 * \dx, 0.75+ 0.5 * \dx + \dy);
                \draw[thick] (4.0 - 0.25 + 0.5 * \dx, -0.75 + 0.5 * \dx + \dy) -- (4.0 + 0.25 + 0.5 * \dx,-0.25+ 0.5 * \dx + \dy);
                \draw[thick] (4.0 - 0.25 + 0.5 * \dx, 0.25 + 0.5 * \dx + \dy) -- (4.0 + 0.25 + 0.5 * \dx, 0.75+ 0.5 * \dx + \dy);
                \draw (4.0 + 0.5 * \dx, 0.0+ 0.5 * \dx + \dy) node{$\bullet$};
            }
        }

        \draw[dashed] (4 - 1, 0) -- (4 - 1,  1);
        \draw[dashed] (4 + 1, 2) -- (4 + 1,  3);        
        \draw[thick, <->] (4 - 1, 1) -- (4 + 1,  3);
        \draw (4, 2.5) node{$h_x$};

        \draw[dashed] (4 + 1, 2) -- (4 + 2,  2);
        \draw[thick, <->] (4 + 2, 2) -- (4 + 2,  0);
        \draw (6.5, 1) node{$h_y$};


        \draw (4.0, 0.0) node{$\bullet$};
        \draw[thick] (0.8,-0.25) arc (-55:15:0.4);
        \draw (1.2, 0.0) node{$\theta$};
        \draw (3.95, -0.25) node{$\vc C$};

        \draw (-0.4,3.75) node{$\vu u$};
        \draw (-2.2,-1.8) node{$\vu r$};
        \draw (6.75,0.4) node{$\vu t$};
    \end {tikzpicture}
    \caption{\label{fig:viewport}Geometry of the viewport. Adapted from \cite{wiki_viewport}.}
  \end {center}
\end {figure}

The width $h_x$ and height $h_y$ of the viewport can be related to the 
distance $d$, field-of-view $\theta$ and the inverse aspect ratio 
$(m-1)/(k-1)$ by 
\begin {eqnarray}
    h_x &=& 2d\tan\frac{\theta}{2}, \\
    h_y &=& h_x\frac{m-1}{k-1},
\end {eqnarray}
where we have assumed square pixels. Neighboring pixels can be related by 
shifting vectors 
\begin {eqnarray}
    \Delta\vc x &=& \frac{h_x}{k - 1}\vu r, \\
    \Delta\vc y &=& \frac{h_y}{m - 1}\vu u.
\end {eqnarray}
Now the position of an arbitrary pixel 
\begin {eqnarray}
    \vc r_{ij} &=& \vc r_{11} + (i - 1)\Delta\vc x - (j - 1)\Delta\vc y,
\end {eqnarray}
where the top-left corner pixel can be written 
\begin {eqnarray}
    \vc r_{11} &=& \vc E + d\:\vu t - \frac{h_x}{2}\vu r + \frac{h_y}{2}\vu u.
\end {eqnarray}
Thus, the direction vector to pixel $(i, j)$ in the frame of the observer
\begin {eqnarray}
    \vu R_{ij}' &=& \frac{\vc r_{ij} - \vc E}{||\vc r_{ij} - \vc E||}.
\end {eqnarray}
The direction vector can be then transformed to a direction vector $\vc R_{ij}$
in a frame approximately fixed to the scene with $(\ref{eq:properdir2})$ by 
replacing the vector beta factor of the observer $\vc\beta$ with $-\vc\beta$.

\subsubsection{Ray-Triangle Intersections: M{\"o}ller-Trumbore Algorithm}
Once we have a direction vector for each pixel, we want to compute the closest 
intersection of the corresponding rays with objects in the scene. In computer 
graphics, material objects in the scene are typically represented with triangles.
The most common algorithm for determination of ray-triangle intersections is 
the \textbf{M{\"o}ller-Trumbore Algorithm} \cite{wiki_moller_trumbore}, which 
involves solution of the intersection point of the ray with the plane containing 
the triangle via application of the Cramer's rule in barycentric coordinates.

Consider the ray 
\begin {eqnarray}
    \vc r(t) = \vc O + t\vc D : t \geq 0
\end {eqnarray}
possibly intersecting the plane $P$ containing the triangle defined by the vertices 
$\vc v_1, \vc v_2, \vc v_3\in\mathbb{R}^3$. Then, a normal vector for the plane can be 
written 
\begin {eqnarray}
    \vc n = (\vc v_2 - \vc v_1)\times (\vc v_3 - \vc v_1)
    = \vc e_1 \times \vc e_2,
\end {eqnarray}
where we denote the two edges connected to $\vc v_1$ with 
$\vc e_1 = \vc v_2 - \vc v_1$ and 
$\vc e_2 = \vc v_3 - \vc v_1$.
The ray will not intersect the plane if $\vc v\cdot\vc n  = 0$ unless $\vc O\in P$.
When $\vc v\cdot\vc n\neq 0$, we wish to find the intersection of the ray with the 
plane and determine whether the intersection is with the triangle. The 
M{\"o}ller-Trumbore Algorithm uses barycentric coordinates $(w, u, v)$ so that 
all points $\vc P\in P$ can be expressed
\begin {eqnarray}
    \vc P(w, u, v) = w\vc v_1 + u\vc v_2 + v\vc v_3.
\end {eqnarray}
For the points of the triangle, the conditions 
\begin {eqnarray}
    \label{eq:moller_cond1}
    w, u, v &\geq& 0, \\
    \label{eq:moller_cond2}
    w + u + v &=& 1
\end {eqnarray}
hold and we can expand
\begin {eqnarray}
    \vc P(u, v) &=& (1 - u - v)\vc v_1 + u\vc v_2 + v\vc v_3
    \\
    &=& \vc v_1 + u(\vc v_2 - \vc v_1) + v(\vc v_3 - \vc v_1).
    \\
    \label{eq:barytri}
    &=& \vc v_1 + u\vc e_1 + v\vc e_2
\end {eqnarray}
Now unless the triangle is degenerate, every intersection between the ray and 
the plane can be expressed using $(\ref{eq:barytri})$. The intersection will 
be with the triangle if the conditions $(\ref{eq:moller_cond1})$, 
$(\ref{eq:moller_cond2})$ hold for a non-negative $t$. At the intersection 
\begin {eqnarray}
    \vc O + t\vc D = \vc v_1 + u\vc e_1 + v\vc e_2.
\end {eqnarray}
This can be reorganized into a linear system of equations 
\begin {eqnarray}
    \label{eq:moller_lin}
    \vc O - \vc v_1 
    =
    ...
    \end {bmatrix}.
\end {eqnarray}
The equation $(\ref{eq:moller_lin})$ can be easily solved with Cramer rule 
$x_i = |A_i|/|A|$ for systems of linear equations $A\vc x=\vc b$, 
where $A_i$ is formed from A by replacing column $i$ with $\vc b$.
To apply the formula, we compute the determinants repeatedly using the 
scalar triple product $|\vc a\:\:\vc b\:\:\vc c| = \vc a\cdot(\vc b\times\vc c)$
\begin{eqnarray}
    |A| 
    &=& 
    \begin {vmatrix}
    ...
    \end {vmatrix}
    = 
    \vc e_1\cdot\vc D\times\vc e_2
    \\
    |A_1| 
    &=& 
    \begin {vmatrix}
        \vc O - \vc v_1 & \vc e_1 & \vc e_2
    \end {vmatrix}
    \:\:\:=
    \begin {vmatrix}
       \vc e_2 & \vc O - \vc v_1 & \vc e_1
    \end {vmatrix}
    = 
    \vc e_2 \cdot (\vc O - \vc v_1)\times \vc e_1
    \\
    |A_2| 
    &=& 
    \begin {vmatrix}
        -\vc D & \vc O - \vc v_1 & \vc e_2
    \end {vmatrix}
    =
    \begin {vmatrix}
        \vc O - \vc v_1 & \vc D & \vc e_2
    \end {vmatrix}
    = 
    (\vc O - \vc v_1)\cdot \vc D \times \vc e_2
    \\
    |A_3| 
    &=& 
    \begin {vmatrix}
        -\vc D & \vc e_1 & \vc O - \vc v_1 
    \end {vmatrix}
    =
    \begin {vmatrix}
        \vc D & \vc O - \vc v_1 & \vc e_1
    \end {vmatrix}
    = 
    \vc D\cdot (\vc O - \vc v_1) \times \vc e_1.
\end {eqnarray}
Thereafter, we can compute parameters for the intersection
\begin {eqnarray}
    t = |A_1|/|A|, \quad u = |A_2|/|A|, \quad v = |A_3|/|A|.
\end {eqnarray}
If $t<0$, the intersection is in the opposite direction to the ray.
The conditions $(\ref{eq:moller_cond1})$ correspond to the following sufficient 
conditions of the intersection being outside the triangle:
\begin {eqnarray}
    u+v > 1, \quad u < 0, \quad v < 0.
\end {eqnarray}
Similarly, the conditions $(\ref{eq:moller_cond2})$ correspond to 
\begin {eqnarray}
    u + v < 0 , \quad u > 1, \quad v > 1.
\end {eqnarray}
However, if $u, v\geq 0$ and $u+v \leq 1$, then also $u+v\geq 0$ and $u,v\leq 1$. Thus, 
when using the conditions to dismiss intersections, checking the latter set after 
the first is redundant.

\subsubsection{Sphere-Ray Intersections}
The points of a sphere with center $\vc c$ and radius $r$ can be expressed 
\begin {eqnarray}
    \label{eq:sphereray_sphere}
    | \vc x - \vc c |^2 = r^2.
\end {eqnarray}
Consider also ray 
\begin {eqnarray}
    \label{eq:sphereray_ray}
    \vc x(d) = \vc o + d\vu u,
\end {eqnarray}
where $\vc o$ is the origin, $\vu u$ is the unit direction vector and $d>0$.
Substitution of the equation for the ray $(\ref{eq:sphereray_ray})$ to 
the equation for sphere $(\ref{eq:sphereray_sphere})$, yields
\begin {eqnarray}
    (\vc o + d\vu u - \vc c) \cdot (\vc o + d\vu u - \vc c) - r^2
    =
    |\vc o + d\vu u - \vc c|^2 - r^2 = 0.
\end {eqnarray}
To be able to apply the quadratic formula, we arrange the terms according powers 
of $d$
\begin {eqnarray}
    d^2|\vu u|^2 + 2d\left[
        \vu u\cdot (\vc o - \vc c)
    ...
    + |\vc o - \vc c|^2 - r^2 = 0
\end {eqnarray}
That is, for a quadratic polynomial $ad^2 + bd + c = 0$, we obtain the expressions  
\begin {eqnarray}
    a &=& |\vu u|^2 = 1, \\
    b &=& 2\vu u\cdot(\vc o - \vc c), \\
    c &=& |\vc o - \vc c|^2 - r^2.
\end {eqnarray}
The solution can then be computed using the quadratic formula  
\begin {eqnarray}
    d 
    &=& 
    ...
    \\
    &=& 
    \frac{-2\left[\vu u\cdot(\vc o - \vc c)\right] 
    \pm 
    \sqrt{[2\vu u\cdot(\vc o - \vc c)]^2 - 4|\vc o - \vc c|^2 + 4r^2)}}{2}
    \\
    &=& 
    -\vu u\cdot(\vc o - \vc c)
    \pm
    \sqrt{|\vu u\cdot(\vc o - \vc c)|^2 - |\vc o - \vc c|^2 + r^2}.
\end {eqnarray}
If we denote
$\Delta = |\vu u\cdot(\vc o - \vc c)|^2 - |\vc o - \vc c|^2 + r^2$,
there are 0, 1 or 2 solutions depending on whether $\Delta<0$, $\Delta=0$ or 
$\Delta>0$, respectively. 

In practical computations, the condition $\Delta=0$ is not likely encountered.

\subsubsection{Depth Buffer}
For each pixel, we wish to draw the triangle corresponding to the closest 
intersection with the ray. Simplest way to do this is to brute force compute 
the intersection with each triangle and select the triangle corresponding to 
the closest one. This leads computational effort $O(n)$, where $n$ is the 
number of triangles.
...
\begin {figure}
  \begin {center}
     \includegraphics [angle=0, width=1.0\columnwidth] {fig/doppler_cubes.eps}
     \caption{\label{fig:doppler_cubes}Observer accelerating towards a grid of 
     cubes.}
  \end {center}
\end {figure} 
\newpage
...
\section{Continuous Spectrum, Brightness and Black-Bodies\label{sec:spectrum_and_radiation}}
The previous two chapters have focused on the special case of monochromatic sources 
and have ignored brightness in the visualizations. Transformation of continuous 
spectrum will lead to frequency-dependent change in brightness, which significantly 
complicates the analysis. We will start by introduction of basic radiometric concepts, 
which will be used in the discussion of transformations of continuous spectrum between 
frames. The most important special case of a continuous spectrum is the black-body 
spectrum, which will be discussed in detail.
...
\textbf{Radiometry} refers to techniques used for the detection and measurement
of electromagnetic radiation and usually focuses on the \textbf{optical range} 
in the electromagnetic spectrum covering wavelengths between $1$ mm infrared 
and $100$ nm ultraviolet. Radiometric quantities describe the distribution of 
energy associated to electromagnetic radiation in space, direction and time.
\subsubsection{Basic Radiometric Quantities}
The energy of the electromagnetic radiation with frequency in the range 
$[\nu, \nu + d\nu]$ crossing a surface element $dA$ in time $dt$ to solid 
angle $d\Omega$ around a direction making angle $\theta$ with the surface normal 
of $dA$ can be written (see Figure \ref{fig:radiance})
\begin {eqnarray}
    dE = I_\nu(r, \theta, \phi)\cos\theta\:dA\:d\nu\:\Omega\:dt,
\end {eqnarray}
where $I_\nu$ is called the \textbf{spectral radiance} or \textbf{specific intensity}.
Note that $I_\nu$ is always non-negative and positive value in one direction 
$(\theta, \phi)$ does not imply a negative value in the opposite direction.

\textbf{Spectral irradiance} or \textbf{spectral flux density} is obtained 
by integrating 
\begin {eqnarray}
    F_{\nu} 
    =
    \oint I_\nu\cos\theta\:d\Omega 
    =
    \int_0^\pi\int_0^{2\pi}I_\nu\cos\theta\sin\theta d\phi d\theta.
\end {eqnarray}
\textbf{Radiance} and \textbf{irradiance} are obtained via integration of 
spectral radiance and spectral irradiance over all frequencies 
\begin {eqnarray}
    I &=& \int_0^\infty I_\nu\:d\nu \\
    F &=& \int_0^\infty F_\nu\:d\nu.
\end {eqnarray}
Irradiance can be related to time-averaged Poynting vector 
\begin {eqnarray}
    F = \langle|\vc S|\rangle \cos\alpha,
\end {eqnarray}
where $\alpha$ is the angle between the Poynting vector $\vc S = \vc E\times\vc H$ 
and the unit normal vector of the surface element.

\textbf{Luminosity} or \textbf{radiant flux} of a closed surface is an absolute 
measure of radiated electromagnetic energy per unit time. Luminosity can be 
obtained by integrating irradiance over the surface 
\begin {eqnarray}
    L = \oint_{\partial V} F\:dA,
\end {eqnarray}
where $\partial V$ is the boundary surface of volume $V$.
...
%per unit projected area per unit solid angle. That is, 
%\begin {eqnarray}
%    L_e 
%    &=&
...
\begin {eqnarray}
    L 
    &=&
    \oint_A\int_\Omega I(\vc r, \theta, \phi)\cos\theta\:d\Omega\:dA
\end {eqnarray}
...
\begin {eqnarray}
    ...
    \pi\oint_A I\:dA.
\end {eqnarray}
If $I$ is independent of position on the surface (e.g. due to equal 
...
\subsection{Brightness and Monochromatic Sources}
...
\begin {eqnarray}
    \label{eq:power_density_mono_transform}
    \boxed{
        \quad
        \diff{L}{\Omega} = \left(
            \frac{\omega}{\omega'}
...
\subsection{Transformation of Continuous Spectra}


\subsubsection{Specific Intensity}
The relativistic aberration transform $(\ref{eq:relativistic_aberration2})$ is 
independent of frequency. Thus, the specific intensity 
$I_\nu'(\vc r', \theta', \phi')$ is transformed into $I_\nu(\vc r, \theta, \phi)$,
where the transformation of the spectrum does not depend explicitly on $\theta$
or $\phi$ but only on power of the factor $(\omega/\omega')$. The factor follows 
from the relativistic Doppler shift formula $(\ref{eq:doppler_relativistic})$ as a 
function of velocity and $\theta$ and is thus constant for all frequencies: 
Any signal limited to the frequency range $[\nu_1', \nu_2']$ is transformed into 
a signal limited to the frequency range 
$[(\omega/\omega')\nu_1', (\omega/\omega')\nu_2']$.

For simplicity and emphasis, we will thus denote
\begin {eqnarray}
    I(\nu) := I_\nu(\vc r, \theta, \phi)
    \quad\sim\quad
    I'(\nu') := I'_\nu(\vc r', \theta', \phi')
\end {eqnarray}
Then, from 
$(\ref{eq:power_density_mono_transform})$, we get the correspondence 
\begin {eqnarray}
    I'(\hat\nu') = \delta(\hat\nu' - \nu')
    \quad\sim\quad
    I(\hat\nu) = \left(\frac{\omega}{\omega'}\right)^4\delta(\hat\nu - \nu),
\end {eqnarray}
where $\delta$ is the Dirac delta distribution and $\nu$ is related to $\nu'$
by the relativistic Doppler transformation $(\ref{eq:doppler_relativistic})$.
This however doesn't imply that a continuous spectrum transforms with the 
factor $(\omega/\omega')^4$. To see this, suppose that 
\begin {eqnarray}
    I'(\hat \nu') = 
    \left\{
        \begin {array}{ll}
    ...
    \right.
\end {eqnarray}
Then for a small $\Delta'$, we expect 
\begin {eqnarray}
    I(\nu)\Delta 
    = 
    I(\nu)\left(\frac{\omega}{\omega'}\right)\Delta'
    =
    \left(\frac{\omega}{\omega'}\right)^4 I'(\nu')\Delta'.
\end {eqnarray}
This yields the transformation rule 
\begin {eqnarray}
    \label{eq:radiance_continuous_transform}
    \boxed{
        \quad 
        I_\nu(\vc r, \theta, \phi) 
        = 
        \left(\frac{\omega}{\omega'}\right)^3I'_{\nu'}(\vc r', \theta', \phi').
        \quad
    }
\end {eqnarray}
That is, continuous spectrum transforms proportionally to the third power of 
frequency ratio from the relativistic Doppler shift $(\ref{eq:doppler_relativistic})$.
The relations between $(\vc r', \vc r)$, $(\theta', \theta)$ and $(\nu', \nu)$ follow
from the Lorentz transformations, the relativistic aberration formula 
$(\ref{eq:relativistic_aberration2})$ and the relativistic Doppler shift formula 
$(\ref{eq:doppler_relativistic})$, respectively.


\subsubsection{Black-Body Radiance and Stars}
It is important to note that distant stars are not bright enough to directly 
stimulate the cone cells responsible for perception of color on the retina of 
the human eye. Instead, the perception of distant stars is based 
on the stimulation of rod cells responsible for dark vision. Thus, any realistic
depiction how unaided human vision perceives a star field will not likely involve 
color. 

The electromagnetic spectrum of stars essentially follow the 
blackbody spectrum complicated slightly by absorption and emission lines. 
However, for visual effects discussed in this section, they can be approximated as ideal black-bodies.
...
For an observer moving at relativistic speeds, the perceived color of bright 
black-body sources can be significantly influenced by the Doppler effect. However, 
since the black-body radiation is not monochromatic but consists of a continuous 
spectrum, the computation of the Doppler effect is no longer trivial. 
...
\begin {eqnarray}
    ...
    \right]^{-1}.
\end {eqnarray}
With $(\ref{eq:radiance_continuous_transform})$, this can be easily transformed 
into $S$ by expanding
\begin {eqnarray}
    I(\nu) 
    &=& 
    \left(\frac{\omega}{\omega'}\right)^3
    \frac{h\nu'^3}{c^2}
    \left[
        \exp\left(
            \frac{h\nu'}{kT_{eff}'}
    ...
    \right],
\end {eqnarray}
where we have defined the \textbf{effective temperature} in $S$
\begin {eqnarray}
...
\end {eqnarray}
Thus, we have shown that any black-body spectrum in $S'$ transforms into a 
black-body spectrum in $S$ with an effective temperature 
$(\ref{eq:effective_temperature_doppler})$. The ratio $(\omega/\omega')$ again 
is the one obtained from the relativistic Doppler shift 
$(\ref{eq:doppler_relativistic})$.
...
To compute the color of a bright star observed by an observer in relative motion, 
...
\begin {thebibliography}{1}
    ...
    \bibitem{wiki_viewport}
    Wikipedia, Ray Tracing (graphics), Available at \\
    \url{https://en.wikipedia.org/wiki/Ray_tracing_(graphics)},
    Retrieved 15.2.2026.
    \bibitem{wiki_moller_trumbore}
    Wikipedia, M{\"o}ller-Trumbore Algorithm, Available at \\
    \url{https://en.wikipedia.org/wiki/M%C3%B6ller%E2%80%93Trumbore_intersection_algorithm},
    Retrieved 2.3.2026.
    \bibitem{wiki_linesphere}
    Wikipedia, Line-Sphere Intersection, Available at \\
    \url{https://en.wikipedia.org/wiki/Line%E2%80%93sphere_intersection},
    Retrieved 21.2.2026.
    \bibitem{cie}
    ...
    \bibitem{srgb}
    IEC, International Standard, IEC 61966 2-1 Amendment 1, 2003.
    \bibitem{meeus}
    Meeus, Astronomical Algorithms, Willmann-Bell, 2nd edition, 1998.
    \bibitem{radiance}
